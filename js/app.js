...
import { initDB, setMeta, getAllPlaces, savePlace, deletePlace, exportAllData, importAllData, login, register, logout, isLoggedIn, uploadPhoto } from './data.js';
...
function bindEvents() {
  ...
  document.getElementById('btn-save-add').addEventListener('click', async () => {
    ...
    if (isEditing && selectedPlaceId) {
      ...
      // 再次保存以持久化照片
      if (newPhotos.length > 0) await savePlace(place);
      places = await getAllPlaces();
    ...
    // 再次保存以持久化照片
    if (newPhotos.length > 0) await savePlace(newPlace);
    places = await getAllPlaces();
    document.getElementById('add-modal').classList.add('hidden');
    updateStats();
    isEditing = false;
